...
let RandomChoice = {
        ...
        for (let i = 0; i < 4; i += 1) {
            ...
            if (index !== -1) {
                ...
                if (!this.visitedLookup.has(JSON.stringify(newMatrix))) {
                    ...
                    this.queuedStates.push([0, 2, newMatrix]);
                }
            }
        }
    ...
    step: function () {
        ...
        if (this.queuedStates.length !== 0) {
            const itemIndex = Math.floor(Math.random() * this.queuedStates.length | 0);
            const [parentId, depth, matrix] = this.queuedStates[itemIndex];
            const isAnswer = JSON.stringify(matrix) === JSON.stringify(this.toMatrix);
            this.queuedStates.splice(itemIndex, 1);
            this.currentId += 1;
                ...
                if (index !== -1) {
                    ...
                    if (!this.visitedLookup.has(JSON.stringify(newMatrix))) {
                        if (JSON.stringify(newMatrix) !== JSON.stringify(this.toMatrix))
                            this.visitedLookup.set(JSON.stringify(newMatrix), 0);
                        this.queuedStates.push([this.currentId, depth + 1, newMatrix]);
                    }
                }
            }
            this.callback(parentId, this.currentId, isAnswer, matrix, depth, null);
    ...
    getMemoryConsumed: function () {
        return this.queuedStates.length * 48 + this.visitedLookup.size * 46;
    }
};
